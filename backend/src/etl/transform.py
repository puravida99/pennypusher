...
class DatetimeTransform(Transform):
    ...
    def __init__(self, col: str, outcol: str, fmt: str = "%Y-%m-%d"):
        super().__init__()
        self.col = col
        self.outcol = outcol
        self.fmt = fmt
...
class StandardizeDates(Transform):
    """
    TODO: Autogenerated by windsurf, need to review. 
    Converts a string column to a datetime column.
    """
...
class StringCleaner(Transform):
    ...
    def __init__(self, col: str, outcol: str, regex: str, replacement: str = ""):
        """
        Args:
...
class InferCurrency(Transform):
    ...
    def apply(self, lf: pl.LazyFrame) -> pl.LazyFrame:
        lf = lf.with_column(
            pl.col(self.amount_col)
            .extract(r"([^\d\s]+", group=1)
            .alias(self.tmp_col)
        )
...
#     """ """
#     # Should this be a transform? It returns a list of classifications which have a label and a score which doesnt work with a column format.
#     # Could flatten the list into multiple columns. Could make this a new class of operation instead, similar to the graph creation.
#     def __init__(self, col: str, outcol: str, categorizer : ICategorizer):
#         super().__init__()
#         self.col = col
#         self.outcol = outcol
